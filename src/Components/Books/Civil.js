...
const Civil = (props) => {
    ...
    useEffect(()=>{
      if(!sessionStorage.getItem('admin') && !sessionStorage.getItem('user') && !sessionStorage.getItem('username'))
      {
        ...
        props.setShow(true)
        const getData = async ()=>{
        let resp = await fetch('http://localhost:8000/books/ce')
  ...
  return (
    <>
        <div className="row">
        <Modal book_model="Civil Engineering" issued_by={sessionStorage.getItem('admin')} image={bookDetail.image} book_id={bookDetail.id} book_name={bookDetail.book_name} book_author={bookDetail.book_author} change={change} setChange={setChange} showAlert={props.showAlert}/>
        {data && data.map((elem)=>{
          return (
